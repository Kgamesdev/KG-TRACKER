...
class RoundedButton(QPushButton):
    def __init__(self, parent=None, text='', command=None, width=120, height=42, bg=COLOR_BG_CARD, hover_bg=COLOR_HOVER, fg='white', radius=12, font=None, border=COLOR_BORDER, border_width=1, icon_path=None, icon_size=(20,20), role='default'):
        ...
        if 'AHORRADO' in text.upper():
            try: import re; self._current_val=float(re.sub(r'[^0-9.-]','',text.replace(',',''))); self._target_val=self._current_val
            except: pass
        ...
        from PySide6.QtCore import QVariantAnimation, QTimer; self._anim=QVariantAnimation(self); self._anim.setStartValue(0.0); self._anim.setEndValue(360.0); self._anim.setDuration(2200); self._anim.setLoopCount(-1); self._anim.valueChanged.connect(lambda v: (setattr(self, '_ang', v), setattr(self, '_shine_pos', -60.0 + (v / 360.0) * 240.0), self.update()))
    ...
    def enterEvent(self, e):
        self._is_hover=True
        if 'AHORRADO' not in self.text().upper(): self._anim.start()
        super().enterEvent(e)
    def leaveEvent(self, e):
        self._is_hover=False
        if 'AHORRADO' not in self.text().upper(): self._anim.stop()
    ...
    def paintEvent(self, e):
        ...
        p.setPen(pen); p.setBrush(__import__('PySide6.QtCore', fromlist=['Qt']).Qt.BrushStyle.NoBrush); p.drawRoundedRect(r, float(self._radius), float(self._radius));
        if 'AHORRADO' in self.text().upper():
            c_bd='#FFD700' if not (self._is_hover or self._flicker_state) else '#FFFF80'; ab=2.5 if (self._is_hover or self._flicker_state) else 1.2; p.setPen(__import__('PySide6.QtGui', fromlist=['QPen']).QPen(__import__('PySide6.QtGui', fromlist=['QColor']).QColor(c_bd), ab)); p.drawRoundedRect(r, float(self._radius), float(self._radius)); lg = __import__('PySide6.QtGui', fromlist=['QLinearGradient']).QLinearGradient(self._shine_pos, 0, self._shine_pos + 35, self.height()); lg.setColorAt(0.0, __import__('PySide6.QtGui', fromlist=['QColor']).QColor(255,255,255,0)); lg.setColorAt(0.5, __import__('PySide6.QtGui', fromlist=['QColor']).QColor(255,255,255,190)); lg.setColorAt(1.0, __import__('PySide6.QtGui', fromlist=['QColor']).QColor(255,255,255,0)); p.setPen(__import__('PySide6.QtCore', fromlist=['Qt']).Qt.PenStyle.NoPen); p.setBrush(lg); p.drawRoundedRect(r, float(self._radius), float(self._radius))
        p.end()
    def setText(self, text):
        if 'AHORRADO' in str(text).upper() and not getattr(self, '_bloqueo_recursivo', False):
            try: import re; nv=float(re.sub(r'[^0-9.-]', '', str(text).replace(',', '')))
            except: nv=0.0
            if abs(nv-self._target_val)>0.01: setattr(self, '_target_val', nv); setattr(self, '_flicker_count', 0); setattr(self, '_flicker_state', True); self._flicker_timer.start() if not self._flicker_timer.isActive() else None; self._count_timer.start() if not self._count_timer.isActive() else None; return
    ...
    def _do_count(self):
        ...
        if abs(d)<0.05: self._current_val=self._target_val; self._count_timer.stop()
        else: self._current_val+=d*0.12
        self._bloqueo_recursivo=True; self.setText(f'$ AHORRADO : {self._current_val:,.2f}'); self._bloqueo_recursivo=False
