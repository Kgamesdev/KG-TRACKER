"""Interfaz grÃ¡fica principal de K GAME TRACKER."""
...
import winsound  # Reemplazado por winsound
...
from core.images import (
    ...
    obtener_tamaÃ±o_cache
)


class RoundedButton(tk.Frame):
    """BotÃ³n con esquinas redondeadas y soporte para icono PNG."""
...
class VentanaPrincipal:
    ...
    def _build_ui(self):
        ...
        # BotÃ³n Todas / Ninguna (estilo principal)
    ...
    # ------------------------------------------------------------------------
    # TIENDAS (estructura original con mÃ¡s altura)
    # ------------------------------------------------------------------------
    ...
    def _inicializar_audio(self):
        try:
            ...
            print(f"âš ï¸ No se pudo inicializar el audio: {e}")

    def _fade_audio(self, porcentaje):
        try:
            porcentaje = max(0, min(10, porcentaje))
    ...
    def cambiar_volumen(self, valor):
        try:
            ...
            print(f"âš ï¸ Error cambiando volumen: {e}")
    ...
    def alternar_mute(self):
        try:
            ...
            print(f"âš ï¸ Error alternando mute: {e}")
    ...
    def buscar_juegos(self):
        limpiar_cache_imagenes()
        self.status_pill.config(text="â— BUSCANDO", fg=COLOR_WARNING)
        ...
        try:
            ...
            if not isinstance(giveaways, list):
                raise ValueError("Formato de API no vÃ¡lido")
            ...
            self.juegos_cache_global = juegos_validos
            ...
            self.status_pill.config(text=f"â— {len(juegos_validos)} OFERTAS", fg=COLOR_SUCCESS)
            self.btn_side_todas.pack()
            self._actualizar_vista_juegos()

        except Exception as e:
            self.status_pill.config(text="â— ERROR", fg=COLOR_ERROR)
            messagebox.showerror("Error", f"OcurriÃ³ un problema de red:\n{e}")
    ...
    def _actualizar_vista_juegos(self):
        ...
        for store in tiendas:
            juegos = [j for j in self.juegos_cache_global if self._asignar_tienda(j) == store]
    ...
    def _crear_tarjeta_juego(self, juego, nombre_tienda):
        titulo = juego.get("title", "Elemento sin tÃ­tulo")
        ...
        btn = RoundedButton(
            card,
            text="RECLAMAR",
            command=lambda u=link_url: self.abrir_enlace(u),
            width=128,
            height=40,
            bg=COLOR_SUCCESS,
        ...
        btn.grid(row=0, column=2, rowspan=2, padx=(2, 12), pady=10, sticky="e")
    ...
    def volver_atras(self):
        ...
        self.juegos_cache_global = []
        self._actualizar_vista_juegos()
        self.status_pill.config(text="â— LISTO", fg=COLOR_SUCCESS)
    ...
    def abrir_ajustes(self):
        ...
        v.title("Ajustes y ConfiguraciÃ³n")
        v.geometry("400x285")
    ...
    def _guardar_autostart(self, habilitar):
        try:
            ...
            messagebox.showwarning("Aviso", f"No se pudo modificar el inicio automÃ¡tico:\n{e}")
